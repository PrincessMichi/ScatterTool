    ...
    def scatter_fx(self):
        ...
        if cmds.objectType(object_to_instance) == 'transform':
            for vertex in vertex_names:
                ...
                # selected_verts = vertex_names

                # scramble the order of the list for more variance
                random.shuffle(vertex_names)

                # Calculate the number of items to select given the percentage and length of all the points
                number_of_points = int(len(vertex_names) * self.dens_sbox.value() / 100)

                # choose at random a percentage of the points
